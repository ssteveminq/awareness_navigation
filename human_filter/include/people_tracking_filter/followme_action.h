...
class Edgeleg_manager_action
{
public:
  /// constructor
  Edgeleg_manager_action(ros::NodeHandle nh, std::string name);

  /// destructor
  virtual ~Edgeleg_manager_action();
 ...
 void execute_follow_action(const human_filter::FollowMeGoalConstPtr &goal);
...
protected:
  actionlib::SimpleActionServer<human_filter::FollowMe> as_; // NodeHandle instance must be created before this line. Otherwise strange error occurs.
  std::string action_name_;
  // create messages that are used to published feedback/result
  human_filter::FollowMe feedback_;
  human_filter::FollowMe result_;
