...
void Dynamic_Manager::filterhumanbelief()
{
    ...
    for (int index = largestIndex; index < Human_Belief_Scan_map.data.size(); index++) {
     ...
     setViewpointTarget(map_coord);
     CoordinateTransform_Rviz_Grid_Human(map_coord[0],map_coord[1],1);
     GoalVector.resize(2);
     GoalVector[0]=map_coord[0];
     GoalVector[1]=map_coord[1];
     m_boolSolve=false;
} 
    ...
    if(viewpub_iters>250){
	    ...
	    else
	    {
		 ...
		 //    geometry_msgs::TransformStamped transform_stamped;
			...
			// cout<<"converted x : "<<tempVec[0]<<", y : "<<tempVec[1]<<std::endl;
			// GazePoint_msg.x=origin_to_robot.position.x;
			// GazePoint_msg.y=origin_to_robot.position.y;
			// GazePoint_msg.z=0.0;
			// GazePoint_msg.x=1.0;
			// GazePoint_msg.y=0.0;
...
void Dynamic_Manager::Publish_filter_measurment(int measurement_type)
{
	...
	for(int idx(0);idx<Filtered_leg_human.size();idx++){
        ...
        pos.name = "leg_laser";
        pos.object_id =new_str_name;
        pos.pos.x = Filtered_leg_human[idx][0];
        pos.pos.y = Filtered_leg_human[idx][1];
        pos.pos.z = 1.0;
        pos.reliability = 0.85;
        pos.covariance[0] = pow(0.01 / pos.reliability, 2.0);
        pos.covariance[1] = 0.0;
