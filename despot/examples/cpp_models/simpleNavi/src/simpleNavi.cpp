...
int SimpleNavi::NumActions() const {
	return 4;
}
...
bool SimpleNavi::Step(State& state, double rand_num, int action,
...
double SimpleNavi::ObsProb(OBS_TYPE obs, const State& state,
	int action) const {
	if (action == A_CHECK) {
...
State* SimpleNavi::CreateStartState(string type) const {
	return new SimpleState(1, Random::RANDOM.NextInt(2));
}

Belief* SimpleNavi::InitialBelief(const State* start, string type) const {
	if (type == "DEFAULT" || type == "PARTICLE") {
		...
		cerr << "[SimpleNavi::InitialBelief] Unsupported belief type: " << type << endl;
		exit(1);
	}
}
...
double SimpleNavi::GetMaxReward() const {
	return 10;
}

class SimpleNaviParticleUpperBound: public ParticleUpperBound {
protected:
...
public:
	SimpleNaviParticleUpperBound(const DSPOMDP* model) {
...
ScenarioUpperBound* SimpleNavi::CreateScenarioUpperBound(string name,
	string particle_bound_name) const {
	ScenarioUpperBound* bound = NULL;
	if (name == "TRIVIAL" || name == "DEFAULT") {
		bound = new TrivialParticleUpperBound(this);
	} else if (name == "MAX") {
		bound = new SimpleNaviParticleUpperBound(this);
	} else {
...
ValuedAction SimpleNavi::GetMinRewardAction() const {
	return ValuedAction(A_EAST, 0);
}

class SimpleNaviEastPolicy: public Policy {
public:
	SimpleNaviEastPolicy(const DSPOMDP* model, ParticleLowerBound* bound) :
		Policy(model, bound) {
	}
...
ScenarioLowerBound* SimpleNavi::CreateScenarioLowerBound(string name,
	string particle_bound_name) const {
	ScenarioLowerBound* bound = NULL;
	if (name == "TRIVIAL" || name == "DEFAULT") {
		bound = new TrivialParticleLowerBound(this);
	} else if (name == "EAST") {
		bound = new SimpleNaviEastPolicy(this,
...
State* SimpleNavi::Allocate(int state_id, double weight) const {
...
State* SimpleNavi::Copy(const State* particle) const {
	SimpleState* state = memory_pool_.Allocate();
...
void SimpleNavi::Free(State* particle) const {
	memory_pool_.Free(static_cast<SimpleState*>(particle));
}

int SimpleNavi::NumActiveParticles() const {
...
void SimpleNavi::PrintState(const State& state, ostream& out) const {
	const SimpleState& simple_state = static_cast<const SimpleState&>(state);
...
void SimpleNavi::PrintObs(const State& state, OBS_TYPE observation,
	ostream& out) const {
	out << (observation ? "GOOD" : "BAD") << endl;
}

void SimpleNavi::PrintBelief(const Belief& belief, ostream& out) const {
...
void SimpleNavi::PrintAction(int action, ostream& out) const {
	if (action == A_SAMPLE)
		out << "Sample" << endl;
	if (action == A_CHECK)
		out << "Check" << endl;
