...
vector<State*> Belief::Sample(int num, vector<State*> particles,
	...
	std::cout<<"unit : "<<unit<<endl;
	std::cout<<"Mass : "<<mass<<endl;
	int pos = 0;

	double cur=0.0;

std::cout<< "Size of Particle :"<<particles.size()<<endl;
	//if(particles.size()>0)  
	cur = particles[0]->weight;
	//double 
	//std::cout<<"curent weight : "<<mass<<endl;
	...
	//	std::cout<<"mass : "<<mass<<endl;
		...
		particle->weight = unit;
		 //  std::cout<<"cur : "<<cur<<"\t";	
		 // std::cout<<"id : "<<particle->state_id<<"\t";
		 // std::cout<<"weight : "<<particle->weight<<endl;
...
void ParticleBelief::Update(int action, OBS_TYPE obs) {
	...
	if (num_effective_particles < num_particles_ / 2.0) {
		cout<<"effective number of particle is small"<<endl;
