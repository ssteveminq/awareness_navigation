...
bool Evaluator::RunStep(int step, int round) {
	...
	cout<<"Runs Step function:"<<endl; 

	double step_start_t = get_time_second();

	double start_t = get_time_second();
	
	//"MK : Her we can get desired policy of action"
	...
	//"MK : Her we can obtain reward and observation"
	...
	*out_ << "-----------------------------------Round " << round
	...
	if (!Globals::config.silence && out_) {
		*out_ << "- Belief = ";
		...
		model_->SaveBelief(*solver_->belief());
	}
...
void Evaluator::publishAction(int action_cmd_)
{
